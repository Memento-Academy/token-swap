...
async function main() {
    ...
    console.log("PEPE:", ethers.utils.formatUnits(pepeReserve, 18));
    console.log("USDC:", ethers.utils.formatUnits(usdcReserve, 6));

    // Add liquidity if needed
    const minPepeLiquidity = ethers.utils.parseUnits("100000000", 18); // 100M PEPE
    const minUsdcLiquidity = ethers.utils.parseUnits("10000", 6); // 10K USDC

    if (pepeReserve.lt(minPepeLiquidity)) {
        console.log("\n💧 Adding PEPE liquidity...");
        const pepeAmount = ethers.utils.parseUnits("1000000000", 18); // 1B PEPE
    ...
    if (usdcReserve.lt(minUsdcLiquidity)) {
        console.log("\n💧 Adding USDC liquidity...");
        const usdcAmount = ethers.utils.parseUnits("100000", 6); // 100K USDC
    ...
    console.log("PEPE:", ethers.utils.formatUnits(finalPepeReserve, 18));
    console.log("USDC:", ethers.utils.formatUnits(finalUsdcReserve, 6));
